...
<div class="banner_area">
            ...
            <form action="{{ route('download-event-certificate') }}" method="POST" onsubmit="return chkcaptchaval();">
                @csrf
                <div class="um-field">
                    <input type="text" name="category" id="category" value="{{ $categories['name'] ?? '' }}" disabled>
                    <input type="hidden" name="category_name" id="category_name" value="{{ $categories['name'] ?? '' }}">
                    <input type="hidden" name="categoriesid" id="categoriesid" value="{{ $categoriesid ?? '' }}">
                    <input type="hidden" name="event_id" id="event_id" value="{{ encrypt($id) ?? '' }}">
                    {{-- <select name="category" id="category" class="fit-pe-inputs">
                        @foreach ($categories as $key => $value)
                          <option value="{{ $value->id }}" <?php //if($value->id == $users->category){ echo 'selected=selected'; }?>>{{$value->name}}</option>
                        @endforeach
                      </select> --}}
                </div>
                            <div class="um-field">
                                ...
                                <select name="cert_type" id="cert_type"  onchange="select_cert_type(this.value)">
                                    @if($categories['name'] == 'Fit India Cyclothon' || $categories['name'] == 'Fit India Cycling Drive')
                                        @if(!empty($users->participant_names))
                                            <option value="organiser">Participant</option>
                                            <option value="participant">Participant Member</option>
                                        @elseif(empty($users->participant_names))
                                            <option value="organiser">Participant</option>
                                            <option value="participant">Participant Member</option>;
                                        @endif
                                    @else
                ...
                <input type="hidden" name="name" value="{{ $users->name }}">
                 <div class="login-row">
                            ...
                            <div class="um-field">
                <div id="show_timeing" class="btn btn-danger btn-lg timer_show w-75 mb-2 " style="display:none ; cursor:not-allowed; ">
                    &nbsp; <span id="timer"> </span>
                </div>
                <button class="btn btn-danger btn-lg timer_show w-75" type="submit" name="submit" id="d_cert_on_button" value="Download Certificate" style="background-color:#FF339F " >Download Certificate</button>
                ...
                <button class="btn btn-danger btn-lg" type="button" name="button" id="d_cert_disable_button" disabled value="Download Certificate" style="background-color:#FF339F; display:none" >Download Certificate</button>
                </br></br>
                <a href="{{url('event-add-partcipant/'.encrypt($id))}}">Click here to upload participant details to download certificate</a>
                </div>
                                       </div>
                                     </div>
            </form>
...
<script>
    ...
    function select_cert_type(el){
        if(el == 'participant'){
            ...
            $("#d_cert_on_button").css("display", "none");
        }else{
...
$('.timer_show').click(function(){

let timerOn = true;

$("#show_timeing").css("display", "block");
$('#show_timeing').off('click');
$("#d_cert_on_button").css("display", "none");
...
function timer(remaining) {
    ...
    // $("#show_timeing").css("display", "block");
    $("#show_timeing").css("display", "none");
    $("#d_cert_on_button").css("display", "block");
