...
@section('title', 'Fit India Admin - Quiz Participants')
@section('content')
<style>
...
<div class="content-wrapper">
	<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Quiz Participants</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
				<li class="breadcrumb-item active" aria-current="page">Quiz Participants</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="row">
		  ...
		  <div class="card">		   
		   <div class="card-header">			  
		     <div class="row">
			  <div class="col-md-2">  
				<button class="btn btn-success btn-sm dwl" name="download" value="download" onclick="window.location.href='quiz_export?uname={{ request()->input('user_name')}}&org={{ request()->input('organisation')}}&st={{ request()->input('state')}}&month={{ request()->input('month')}}&role={{ request()->input('role')}}&search=search';">
				 <i class="fa fa-download"></i> Download</button>				 
			  </div>
			  <div class="col-md-10">
			  <form class="form-inline my-2 my-lg-0 rtside" type="get" action="{{ route('admin.quizs.index') }}">
                <div class="form-group rtside">
					...
					<select class="custom-select custom-select mb-3" name="organisation" id="partner" style="width:175px" >
					  <option value="">Select Partner</option>       
					   <?php
					    if(!empty($partnerdata)){
					      foreach($partnerdata as $pdata){							  
                            if(!empty($_REQUEST['organisation'])&& $_REQUEST['organisation']==$pdata->uid){
							   $ptselect='selected';
							 }else{
							   $ptselect='';
							 }
						 ?>						 
						 <option <?=$ptselect?> value="{{ $pdata->uid }}">{{ $pdata->uname }}</option> 
						<?php } } ?> 					 
                     </select>
			...
			<div class="col-md-6">
			  ...
			  ?>
			  Total Quiz : <strong>{{ $curcount }}/{{ $count }}</strong>			 
			</div>
