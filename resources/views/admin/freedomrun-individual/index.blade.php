        ...
        <div class="row">
          <div class="col-md-12">		  
		   <div class="card">		   
			  ...
			  <div class="row">
			  <div class="col-md-2">			 
			  <button class="btn btn-success btn-sm dwl" name="download" value="download" onclick="window.location.href='freedomrun_export?ename={{ request()->input('event_name')}}&type=individual&search=search';">
			  <i class="fa fa-download"></i> Download</button>			
			  </div>
			  <div class="col-md-10">
			  <form class="form-inline my-2 my-lg-0 rtside " type="get" action="{{ route('admin.freedomrun-individual.index') }}">
			...
			<div class="col-md-6">
			 ...
			 Total Freedomrun : <strong> {{ $count }}</strong>			 
			</div>		
         ...
         <div class="card-body table-responsive p-0">
		  <table class="table table-striped projects">
              ...
              <tbody>
			   <?php $i=0; ?>
             @if(count($freedomrun)>0)
                  @foreach($freedomrun as $freedom)
                  <tr>
                      <th scope="row">{{++$i}}</th>
					  <td>@if(!empty($freedom->name)) {{ ucwords(strtolower($freedom->name)) }}  @endif</td>
                      <td>@if(!empty($freedom->email)) {{ $freedom->email }} @endif</td>
                      ...
                      <!--<td>@if(!empty($freedom->participantnum)) <br>{{ $freedom->participantnum }}  @endif</td>-->					  
                      <td>@if(!empty($freedom->role)) <br>{{ $freedom->role }}  @endif</td>					  
                      <td style="width:120px;display:inline-flex !important;">  
                        &nbsp;<a style="display: inline !important;" class="btn btn-info btn-xs" href="{{ route('admin.freedomrun-individual.edit', $freedom->id) }}"> <i class="fas fa-pencil-alt"></i></a>
                        &nbsp;&nbsp;
                        <!--<form action="{{ route('admin.freedomrun-individual.destroy', $freedom->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
