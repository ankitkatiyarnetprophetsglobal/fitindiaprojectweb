...
$("#frontadmin").on("submit", async function (e) {
    ...
    if (!email || email.length > 255 || !/^[a-zA-Z0-9@._-]+$/.test(email)) {
        $("#email").after(
            '<span class="error-msg" style="color:red; font-size:0.85em;">Please enter a valid email address.</span>'
        );
        hasError = true;
    }
    ...
    if (!password || password.length < 6 || password.length > 255) {
        $("#password").after(
            '<span class="error-msg" style="color:red; font-size:0.85em;">Password must be between 6 and 255 characters.</span>'
        );
        hasError = true;
    }
    ...
    if (!captcha || captcha.length > 10 || !/^[a-zA-Z0-9]+$/.test(captcha)) {
        $("#captcha").after(
            '<span class="error-msg" style="color:red; font-size:0.85em;">Please enter a valid captcha.</span>'
        );
        hasError = true;
    }
                ...
                if (xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    $.each(xhr.responseJSON.errors, function (key, messages) {
                        if (key === "emailTomanyattampt") {
                            ...
                            $(`#${key}`).after(
                                `<span class="error-msg" style="color:red; font-size:0.85em;">${messages[0]}</span>`
                            );
                        }
                    });
                } else {
...
$("#reload").click(function () {
    ...
    jQuery.ajax({
        type: "GET",
        // url: routes.reloadCaptcha, // Use named route instead of hardcoded URL
        url: 'https://fitindia.gov.in/reloadcaptcha', // Use named route instead of hardcoded URL
        headers: {
            csrfToken,
        },
