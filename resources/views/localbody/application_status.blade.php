								...
								<div class="wrap event-form">	
									...
									@if(!empty($prerak_data))
							 		<table class="table table-bordered" style="margin-bottom:0!important; ">
										...
										<th>Applied for</th>
										<td>
										Fit India Urban Local Body Ambassador
										</td>
										</tr>
										...
										<tr>
										<th>Designation</th>
										<td><?=ucwords(strtolower($prerak_data->designation))?></td>
										</tr>
										<tr>
										<th>Gender</th>
										...
										<tr>
										<th>Block Name</th>
										<td><?=ucwords(strtolower($prerak_data->block_name))?></td>
										</tr>
										
										<tr>
										<th>Letter / Document</th>
										<td><a href="<?=$prerak_data->document_file?>" download>Click to download Letter / Document</a></td>
										</tr>
                					...
                					@if($prerak_data->status=='1')
										...
										<div style="    bmargin-bottom:15px;">
											<fieldset class="scheduler-border" id="event_details" style="border: 1px solid #dee2e6;padding:15px;">
                                                ...
                                                <form method="POST" action="update_localbody_newevent" enctype="multipart/form-data">
                                                	@csrf
										...
										<div class="um-field">
											<div class="um-field-area">
												<a target="_blank" class="btn btn-primary flag-dwn" href="download-loalbody-certificates">Download Fit India Urban Local Body Ambassador Certificate</a>
											</div>
										</div>
										<?php } ?>
									@endif
									...
									<?php if($_REQUEST['auth_id']){ ?>
										<a href="{{url('/lbapply-again-mobile')}}/?auth_id=<?=@$_REQUEST['auth_id']?>&event_id=<?=@$prerak_data->id?>" onclick="return confirm('Your old data registered as Fit India <?=$prerak_data->register_as?> will be removed');">Click here to apply again</a>
									<?php	}else{ ?> 
										<a href="{{url('/localbody-apply-again')}}/{{$prerak_data->id}}" onclick="return confirm('Your old data registered as Fit India <?=$prerak_data->register_as?> will be removed');">Click here to apply again</a>
									<?php } ?>
									@endif
								@endif
							</div>
