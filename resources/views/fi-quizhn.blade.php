...
<head>
  <title>{{ ucwords($userdata->name) }} रोड टू टोक्यो 2020 प्रश्नोत्तरी</title>
  <meta charset="utf-8">
...
<body oncopy="return false" oncut="return false" onpaste="return false">
 ...
 <form action="{{ route('quizstorehn') }}" method="POST" onsubmit="return checkquiz()" name="quiztokyo" >
 @csrf
      ...
      <div class="row ">
        <div class="col-sm-12 col-md-12 col-lg-12 no-padding">
          ...
          <div class="countermtr"> <span id="minutes"></span> :  <span id="seconds"></span> </div>
            <h1> 
			{{ ucwords($userdata->name) }} - रोड टू टोक्यो 2020 प्रश्नोत्तरी   
            </h1>
...
-->
              ...
              <div id="div_userdata" class="user_area_q">
				...
				<input type="hidden" name="sec" id="sec" />
                  <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="नाम" value="{{ old('name') }}" >
				  @error('name')
                ...
                <div class="form-group user_child">
                  <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" placeholder="ईमेल" value="{{ old('email') }}">
				  @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
				...
				<div class="form-group user_child">
					<input type="text" name="phone" id="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="मोबाइल" value="{{ old('phone') }}">
					@error('phone')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
				...
				<div class="form-group user_child last-t">
					...
					<input type="text" name="city" id="city" class="form-control @error('city') is-invalid @enderror" placeholder="शहर" value="{{ old('city') }}">
					@error('city')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
				...
				<div class="form-group" style="margin: 10px 6px;">
					<span class="start-btn" onclick="checkuser()" id="start-btn" >Start Quiz</span>
				</div>
		...
		<div class="row bg_c bg_c_r">
			...
			<div class="col-sm-12 col-md-12 col-lg-12">
			  <div class="main-content">
				...
				<div class="seq_que">
				  <h3>प्रश्नोत्तरी शुरू करने के लिए उपरोक्त विवरण भरें </h3>
				</div>
  ...
  <script>
	...
	function checkuser(){
		...
		if(sclass){ 
			...
			if(!checking){
				msgflag = true;
				msg += ' मोबाइल फोन 10 अंक होना चाहिए ';
					...
					success: function(response){
						...
						for (let i in response) {
						  //console.log(response[i].id + " ");
							cnt++;
							msg +='<div class="row bg_c bg_c_r"> <div class="col-sm-12 col-md-12 col-lg-12"> <div class="main-content"> <div class="ques_d">प्रश्न:<span>'+cnt+'</span></div> <div class="seq_que"> <h3>'+response[i].ques+'</h3> </div> </div> </div>';
						...
						msg += '<div class="button-box_c"> <input type="submit" id="btnSubmit" name="qizSubmit" class="next mt-4  mb-4" value="भेजें"</div>';
