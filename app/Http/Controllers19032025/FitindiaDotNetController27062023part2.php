...
class FitindiaDotNetController extends Controller
{
        ...
        try{
            ...
            if ($data == "" || $data == null){
                ...
                if ($decryptedData == "" || $decryptedData == null){
                    ...
                    $dataresult->execute();           
                    $dataresultshow = $dataresult->fetchAll(PDO::FETCH_ASSOC);
                    $data = (object)$dataresultshow[0];                    
                    // dd($data->F365Id);
                    if ($Output == 0){
                        ...
                        if($data->ParentId == null){
                            ...
                            $ParentId = $data->F365Id;
                        }
                    
                        $F365Id = $data->F365Id;
                        $type = 1;
                        $request->session()->put('lang', $Language);
                        $request->session()->put('ParentUserId', $data->F365Id);
                        ...
                        $request->session()->put('MobileNo', $data->MobileNo);
                        $request->session()->put('KheloIndiaId', $data->KheloIndiaId);
                        $request->session()->put('ErrorCode', 0);

                        // $userinformation = $db_ext->select("EXEC spGetFitnessUser $F365Id,$type");

                        $dataresultFitnessUser = $conn->prepare("EXEC spGetFitnessUser $F365Id,$type");
                        $dataresultFitnessUser->execute();           
                        $userinformation = $dataresultFitnessUser->fetchAll(PDO::FETCH_ASSOC);
                        // $userinformation = (object)$dataresultFitnessUser[0];
                        // dd($userinformation);
                        foreach($dataresultFitnessUser as $x => $val) {                        
    ...
    public function homelistview(Request $request){
        ...
        try {
            ...
            $conn = new PDO("sqlsrv:Server=$mssqluser_hostname,1433;Database=$mssqluser_databases", "$mssqluser_name", "$mssqluser_password");
            // dd($conn);
            $dataresult = $conn->prepare("EXEC spGetFitnessUser $userid,$type");
            $dataresult->execute();           
            ...
            if($ParentId != null){        
                ...
                $dataresult = $conn->prepare("EXEC spGetFitnessUser $ParentId,$typedata");
                $dataresult->execute();           
                $datacount = $dataresult->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function storedata(Request $request){
        ...
        try {
            ...
            $mssqluser_name= $this->DB_SQL_USERNAME;
            $conn = new PDO("sqlsrv:Server=$mssqluser_hostname,1433;Database=$mssqluser_databases", "$mssqluser_name", "$mssqluser_password");
            $dataresult = $conn->prepare("EXEC spKheloIndiaFitnessSignup $F365Id,$KheloIndiaId,"."'$UserName'".","."'$FirstName'".","."'$MobileNo'".","."'$email'".","."'$dob'".",$GenderGroup,"."'$AddressLine1'".","."'$AddressLine2'".",$District,$State,$PinCode,"."'$PreferredSports'".",$UserPrivacyAcceptance,"."'$UserPrivacyAcceptanceDateTime'".",$UIDTypeId,"."'$UIDNo'".","."'$disability'".",$parentid,"."'$PrivacyAcceptedVersion'".","."'$PhotoPath'".","."'$PhotoName'".","."'$ContentType'".",$Data");
            $dataresult->execute(); 
            // dd($dataresult);
            // $stmt ""
    ...
    public function getalldata(Request $request, $ParentId){
        ...
        try {
            ...
            $mssqluser_name= $this->DB_SQL_USERNAME;   
            $conn = new PDO("sqlsrv:Server=$mssqluser_hostname,1433;Database=$mssqluser_databases", "$mssqluser_name", "$mssqluser_password");
            $dataresult = $conn->prepare("EXEC spGetFitnessUser $ParentId,$type");
            $dataresult->execute();           
            $data = $dataresult->fetchAll(PDO::FETCH_ASSOC);     
