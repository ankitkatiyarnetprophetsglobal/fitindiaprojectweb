...
class FitindiaDotNetreportController extends Controller
{
    ...
    public function fitindiareport(Request $request){
        ...
        try{
            ...
            // $stmt = $conn->prepare("fitindiacarnivaltestresult");
            ...
            $stmt1 = $conn->prepare(("EXEC fitindiacarnivaltestresult"));
            ...
            // dd("1");
            foreach ($data1 as $each_user) {
                // dd($each_user->Test_Date);
            ...
            // dd("ankit");
            //download command
            return Response::download($filename, "BMIdata.csv", $headers);
            // dd($data1);

            $stmt2 = $conn->prepare(("EXEC fitindiacarnivaltestresult"));
            $stmt2->execute();
            $data2 = $stmt2->fetchAll(PDO::FETCH_OBJ);
            // dd($data2);            
            
            // return view('static.report', [
            //     'data' => $data1,
            //     'countdatacount' => $data2,
            //     'usermetas_image' => $usermetas_image
            // ]);
    ...
    public function fitindiareportfitnesstest(Request $request){
        ...
        try{
            ...
            foreach ($data2 as $each_user) {
                // dd($each_user['DisplayDate']);
                
                fputcsv($handle, [

                    $each_user['DisplayDate'],
                    $each_user['DATE'],
                    $each_user['F365Id'],
                    $each_user['Test_Category_ID'],
                    $each_user['TestName'],
                    $each_user['Score_Criteria'],
                    $each_user['Score_Measurement'],
                    $each_user['Score'],
                    $each_user['Percentile']
            ...
            fclose($handle);           
            // dd(1);
            return Response::download($filename, "fitnesstestreport.csv", $headers);
