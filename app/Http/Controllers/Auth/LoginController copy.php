    ...
    */
    ...
    public function verifyOTP(Request $request)
    {
           ...
           $user = User::find($userId);
            ...
            $otp_v = trim($request->otp);
            $otp_valuecrypt_trim = $this->encrypt($key, $iv, $otp_v);
            $otp_value_trim = $this->decrypt($key, $iv,$otp_valuecrypt_trim);
            $key = $currentdatetime . 'fitind';
            $email_trim = trim($user->phone);
        ...
        if ($request->otp === $otp_value_trim) {
            ...
            if ($user && Auth::attempt($credentials)) {
                ...
                return response()->json([
                    ...
                    'redirect' => route('create-event') // or wherever you want to redirect
                ]);
            }
        }
