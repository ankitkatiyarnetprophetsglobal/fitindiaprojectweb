...
use App\Models\Conclaveregistrations;
use App\Models\Nationawellnessconclaves;
use App\Models\User;
...
class GeneralController extends Controller
{
		...
		try{
            ...
            if(isset($phone)){
                ...
                if(count($records) == 0){
                    ...
                    $otpDailyCount = OtpTrack::where('phone', $phone)
                        ->where('type', 'user')
    ...
    */
	...
	public function mobile_otp_check(Request $request){
		...
		try{
            // dd($request->all());
            // dd($request->captchaotp, session('captcha'));
            $request->validate([
                'captchaotp' => 'required|captcha'
            ], [
                'captchaotp.captcha' => 'Please fill correct value.'
            ]);
			...
			$emailencrypt_trim = $this->encrypt($key, $iv, $email_trim);
            // echo  $reqtimeencrypt_trim;
            // echo  $emailencrypt_trim;
            // echo  $otp_valuecrypt_trim;
            // exit;
			// $apiurl = config('app.api_url').'verifyingemail';
			// $response = Http::post($apiurl, [
			...
			// $response = Http::post('https://service.fitindia.gov.in/api/v2/verifyingemail', [
						...
						'reqtime' => "kEXRJ+hQFjDHQoIjk4YACg==",
						'email' => "YaY5x91TJsR7j0iANYu/Ig==",
						'otp' => "AapRTzLxWCv+XZzzBA2uKA==",
				]);
			...
			if($response['success'] == true){
                ...
                if($mobile == $email_trim){
                    ...
                    $curenttime = time();
                    // $encrypted = $this->cryptoJsAesEncrypt("passphrasepass", "true:truewrtsuss:$curenttime");
                    $encrypted = $this->cryptoJsAesEncrypt("passphrasepass", "true:truewrtsuss:$curenttime:$mobile");
	...
	public function conclave_registration(Request $request){
        ...
        try {
            ...
            return view('conclave-registration', compact('states', 'roles', 'districts'));
        } catch (Exception $e) {
            return abort(404);
        }
    }

    public function save_conclave_registration(Request $request){
        ...
        try {
            ...
            $request->validate([
                ...
                'fullname' => 'required|string|min:3|max:100|regex:/^[a-zA-Z\s\.\'-]+$/',
                'company' => 'required|string|min:3|max:100|regex:/^[a-zA-Z\s\.\'-]+$/',
                'mobile' => 'required|digits:10|regex:/^[6-9]\d{9}$/',
                ...
                'fullname.regex' => 'Name should contain only letters, spaces, or . - \' characters.',
                'company.required' => 'Company can not be blank.',
                'designation.required' => 'designation can not be blank.',
                'mobile.regex' => 'Please enter a valid Indian mobile number.',
                'email.unique' => 'This email is already registered.',
                'captcha.captcha' => 'Invalid captcha, please try again.'
            ]);

            $yourstory = new Conclaveregistrations;
            $yourstory->name = $request->fullname;
            $yourstory->company = $request->company;
            $yourstory->designation = $request->designation;
            $yourstory->email = $request->email;
            $yourstory->mobile = $request->mobile;
            $yourstory->status = 1;
            ...
            return back()->with('message', "Thank you for registering for the Sports Goods Manufacturers Conclave. Your registration has been received successfully.");
        } catch (Exception $e) {
            return $e->message();
            return abort(404);
        }
    ...
    public function national_fitness_and_wellness_conclave(Request $request){
        ...
        try {
            ...
            return view('national-fitness-and-wellness-conclave', compact('states', 'roles', 'districts'));
        } catch (Exception $e) {
            return abort(404);
        }
    }

    public function save_national_fitness_and_wellness_conclave(Request $request){
        ...
        try {
            ...
            $yourstory = new Nationawellnessconclaves;
            $yourstory->name = $request->fullname;
            $yourstory->company = $request->company;
            $yourstory->designation = $request->designation;
            $yourstory->email = $request->email;
            ...
            return back()->with('message', "Thank you for registering for the National Fitness and Wellness Conclave. Your registration has been received successfully.");
        } catch (Exception $e) {
            return $e->message();
            return abort(404);
        }
