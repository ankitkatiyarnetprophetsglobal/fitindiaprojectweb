...
class CyclingeventExport implements FromCollection, WithHeadings
{
    public function collection(){
        ...
        if(request()->has('registrationcyclingevent')){
        ...
        }elseif(request()->has('reportrolewisecyclingevent')){            
            ...
            $records = DB::table('users')
                ...
                return $records;
        }elseif(request()->has('reportdatewisecyclingevent')){

            $records = DB::table('users')
                            ->where([
                                ['users.created_at', '>', '2024-11-15 00:00:01'],
                                // ['role', '=', 'namo-fit-india-cycling-club'],
                                ['role', '=', 'namo-fit-india-youth-club'],
                ...
                // dd($records);
                return $records;
        }elseif(request()->has('reportwithimagecyclingevent')){

            $records = DB::table('event_organizations')
                            ->where("event_organizations.category",'=', 13075)
        ...
        }elseif(request()->has('reportstatewisecyclingevent')){
            ...
            $records = DB::table('users')
                            ...
                            ->where([
                                ...
                                ['role', '=', 'namo-fit-india-cycling-club'],                                
                                ['state', '!=', ''],
                                ['event_organizations.category', '=', '13075']
                            ])
